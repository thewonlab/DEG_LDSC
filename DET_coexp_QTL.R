...
outputdir = "/proj/hyejunglab/crossdisorder/LDSC/partherit/SCZ_expression/DET_coexp_QTL/"
setwd(outputdir)
load("/proj/hyejunglab/chr/geneAnno_allgenes.rda")
load("/ms-old/depts/genetics/hyejunglab/eQTL/capstone4/sQTL/sQTL_GRanges_annotation_list.rda")
...
for(i in 1:length(isoforms)){
  isoform = isoforms[i]
  genes[[i]] = unique(det[det$Module==isoform,"ensembl_transcript_id"])
...
for (i in 1:22) {
  ...
  snpranges = GRanges(paste0("chr",snps.table$CHR), IRanges(snps.table$BP,snps.table$BP))
  ...
  for(j in 1:length(isoforms)){
    ...
    detranges = qtlranges[qtlranges$transcript %in% genelist]
    olap = findOverlaps(snpranges, detranges)
    outframe = snps.table[,1:4]
    outframe$det = 0
    outframe[unique(queryHits(olap)),"det"] <- 1
...
outdir = "/proj/hyejunglab/crossdisorder/LDSC/partherit/SCZ_expression/DET_coexp_QTL/"
condition = "scz3"
isoforms = paste0("isoM",1:56)
...
pdf("isoform_coexp_PEC_QTL.pdf", height=10, width=10)
