...
outputdir = "/proj/hyejunglab/crossdisorder/LDSC/partherit/SCZ_expression/CMC_coexp_HiC/"
setwd(outputdir)
...
for (i in 1:22) {
  ...
  for(j in 1:length(isoforms)){
    ...
    outframe$deg = 0
    outframe[outframe$SNP %in% snplist,"deg"] = 1
    print(sum(outframe$deg))
...
outdir = "/proj/hyejunglab/crossdisorder/LDSC/partherit/SCZ_expression/CMC_coexp_HiC/"
condition = "scz3"
isoforms = paste0("M",1:35,"c")
...
setwd(outdir)
isoAsso = read.csv("/proj/hyejunglab/expression/SCZ_RNAseq/SCZ_CMC_module_definition.csv")
#isoAsso = isoAsso[grep("M", isoAsso$Module),1:7]
#isoAsso$beta = log(isoAsso$Odds.Ratio)
#isoAsso[isoAsso$beta==-Inf, "beta"] <- 0
isoAsso$Module = factor(isoAsso$Module,levels=rev(isoforms))
isoAsso$Group = "SCZ"

heritability$Category = factor(heritability$Category,levels=rev(isoforms))

plot1 = ggplot(isoAsso,aes(Group,Module)) + geom_tile(aes(fill=Odds.Ratio), colour="grey") + coord_fixed(ratio=1) + scale_fill_gradient2(low="steelblue", mid="white", high="indianred", midpoint=) + ylab("Isoform Module")
...
pdf("gene_coexp_CMC_HiC.pdf", height=10, width=10)
